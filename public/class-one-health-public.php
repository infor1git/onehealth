...
class One_Health_Public {
		...
		<div class="bw-wizard-wrapper <?php echo esc_attr($tema_ativo); ?>" id="gh-booking-wizard">
            <div class="bw-wizard-content-inner">
				...
				<div class="bw-wizard-header">
					<?php if($logo_url): ?>
						<img src="<?php echo esc_url($logo_url); ?>" alt="Logomarca" class="bw-wizard-logo">
					<?php else: ?>
						<h2 class="bw-wizard-title">Agendamento Online</h2>
					<?php endif; ?>
				</div>

                <div class="bw-progress-bar">
                    <div class="bw-step active" data-step="1">1. Unidade</div>
                    <div class="bw-step" data-step="2">2. Especialidade</div>
                    <div class="bw-step" data-step="3">3. Serviço</div>
                    <div class="bw-step" data-step="4">4. Convênio</div>
                    <div class="bw-step" data-step="5">5. Profissional</div>
                    <div class="bw-step" data-step="6">6. Horário</div>
                    <div class="bw-step" data-step="7">7. Confirmação</div>
                </div>
                ...
                <div class="bw-wizard-content">
                    ...
                    <div class="bw-step-content" id="step-5">
                        ...
                        <div style="margin-top:20px; text-align:center;">
                            <button type="button" id="gh-skip-medico" class="bw-btn-secondary" onclick="skipMedico()">
                                <span class="dashicons dashicons-groups"></span> Mostrar qualquer profissional
                            </button>
                        </div>
                    </div>
						...
						<div class="bw-calendar-layout">
							<div class="bw-calendar-col">
								<div class="bw-cal-header">
									<button type="button" class="bw-cal-nav" id="bw-cal-prev"><span class="dashicons dashicons-arrow-left-alt2"></span></button>
									<strong id="bw-cal-month-name">Mês</strong>
									<button type="button" class="bw-cal-nav" id="bw-cal-next"><span class="dashicons dashicons-arrow-right-alt2"></span></button>
								</div>
								<div class="bw-cal-days-header">
							...
							<div class="bw-slots-col">
								<div id="gh-slots-list" class="bw-slots-grid">
									<p style="opacity:0.7;">Selecione uma data para ver os horários.</p>
								</div>
							</div>
						</div>
                    </div>
                    ...
                    <div class="bw-step-content" id="step-7">
                        ...
                        <div class="bw-summary">
                            ...
                            <p>Convênio: <span id="sum-convenio"></span></p>
					...
					<div class="bw-navigation-footer" style="margin-top: 30px; border-top: 1px solid var(--bw-color-card-border); padding-top: 15px;">
						<button type="button" class="bw-btn-back" onclick="gh_prev_step()">
                            <span class="dashicons dashicons-arrow-left-alt2"></span> Voltar Passo Anterior
                        </button>
					</div>
            ...
            <div id="bw-modal-preparo" class="bw-modal-overlay" style="display: none;">
                <div class="bw-wizard-content-inner bw-modal-content">
                    <h3 style="margin-top:0; border-bottom: 1px solid var(--bw-color-card-border); padding-bottom: 15px; margin-bottom: 20px;">
                        <span class="dashicons dashicons-clipboard" style="font-size:24px; width:24px; height:24px; vertical-align:middle; color:var(--bw-color-accent);"></span> 
                        Instruções de Preparo
                    </h3>
                    
                    <div id="bw-modal-preparo-text" style="max-height: 50vh; overflow-y: auto; margin-bottom: 20px; line-height: 1.6; padding-right: 10px;"></div>
                    
                    <div class="bw-modal-aviso" style="font-size: 0.9rem; color: var(--bw-color-text-primary); margin-bottom: 25px; padding: 12px; background: rgba(0,0,0,0.1); border-left: 4px solid var(--bw-color-accent); border-radius: 4px;">
                        <span class="dashicons dashicons-info" style="vertical-align: middle; color:var(--bw-color-accent);"></span>
                        <strong>Aviso:</strong> Estas instruções também serão enviadas para o seu e-mail após a conclusão do agendamento.
                    </div>
                    
                    <div style="display: flex; gap: 15px; justify-content: flex-end;">
                        <button type="button" class="bw-btn-back" id="bw-btn-fechar-modal" style="margin:0;">Cancelar</button>
                        <button type="button" class="bw-btn-primary" id="bw-btn-continuar-modal" style="width:auto; margin:0;">Estou Ciente e Continuar</button>
                    </div>
                </div>
            </div>
    ...
    public function ajax_get_servicos() {
        ...
        // [MODIFICADO] Adicionada a coluna preparo_html
        $sql = "SELECT s.id, s.nome, s.valor, s.preparo_html FROM {$wpdb->prefix}gh_servicos s
    ...
    public function ajax_get_convenios() {
        global $wpdb;
        $sql = "SELECT id, nome, logo_url FROM {$wpdb->prefix}gh_convenios WHERE is_active = 1 ORDER BY nome ASC";
        $results = $wpdb->get_results( $sql );
        wp_send_json_success( $results );
    }

    // [BLINDAGEM] MÉTODOS DE CALENDÁRIO OMITINDO AGENDAMENTOS EXISTENTES (STATUS 'A')
	public function ajax_get_available_dates() {
        ...
        // JOIN COM gh_agendamentos para remover da lista qualquer dia que os horários já tenham o Status 'A'
		$sql = "SELECT DISTINCT DATE(s.data_hora) as data_disp 
                FROM {$wpdb->prefix}gh_slots s
                LEFT JOIN {$wpdb->prefix}gh_agendamentos a ON s.id = a.slot_id AND a.status = 'A'
				WHERE s.status = 'disponivel' AND a.id IS NULL 
                AND s.unidade_id = %d AND s.especialidade_id = %d AND s.data_hora > %s AND s.data_hora LIKE %s";
	...
	public function ajax_get_slots() {
        ...
        // JOIN COM gh_agendamentos para ocultar os horários de vez
		$sql = "SELECT s.id, s.data_hora, m.nome as medico_nome 
                FROM {$wpdb->prefix}gh_slots s
				INNER JOIN {$wpdb->prefix}gh_medicos m ON s.medico_id = m.id
                LEFT JOIN {$wpdb->prefix}gh_agendamentos a ON s.id = a.slot_id AND a.status = 'A'
				WHERE s.status = 'disponivel' AND a.id IS NULL 
                AND s.unidade_id = %d AND s.especialidade_id = %d AND DATE(s.data_hora) = %s AND s.data_hora > %s";
	...
	public function ajax_get_unidades() {
		...
		if(file_exists($path)) { require_once $path; $model = new GH_Unidade(); wp_send_json_success( $model->get_all() ); }
		wp_send_json_error();
	}

	public function ajax_get_especialidades() {
		$path = plugin_dir_path( __FILE__ ) . '../includes/models/class-gh-especialidade.php';
		if(file_exists($path)) { require_once $path; $model = new GH_Especialidade(); wp_send_json_success( $model->get_all() ); }
		wp_send_json_error();
	}
	...
	public function ajax_save_booking() {
        ...
        // 1. O slot existe mesmo?
		$slot = $wpdb->get_row( $wpdb->prepare("SELECT * FROM {$wpdb->prefix}gh_slots WHERE id = %d AND status = 'disponivel'", $slot_id) );
		if ( ! $slot ) wp_send_json_error( 'Este horário acabou de ser reservado. Por favor, escolha outro.' );
		
        // 2. [A BARREIRA MESTRA] Existe algum agendamento ativo com status 'A' neste slot?
        $table_agendamentos = $wpdb->prefix . 'gh_agendamentos';
        $ja_agendado = $wpdb->get_var( $wpdb->prepare("SELECT id FROM {$table_agendamentos} WHERE slot_id = %d AND status = 'A'", $slot_id) );
        
        if ( $ja_agendado ) {
            wp_send_json_error( 'Desculpe! Outro paciente acabou de reservar este exato horário nos últimos segundos. Escolha outro.' );
        }
		...
		$data = array(
            ...
            'status'           => 'A', // [ATUALIZADO] Status salvo como 'A'
        ...
        // Atualiza a tabela de slots para evitar aparecer no cache da query principal
